...
_Note:_ This api key is public, and can be submitted to version control. Its better to put it to `.env` file though. There is also a private key, but that should never be committed!
...
First, create a `.env` file (see `.env.example`), and set the variables. If you want to use Algolia Search, you need to [register](https://www.algolia.com/doc/guides/getting-started/what-is-algolia/) and generate your API credentials.
If you don't want to use Algolia, set `enableSearch` to false in `config/site.config.js`.
